...
class IntelligentFrameExtractor:
    ...
    def extract_representative_frames(
        ...
        for i, scene in enumerate(scenes_to_process):
            ...
            end_frame = (
                scene[1].get_frames() if len(scene) > 1 else start_frame + int(fps * 2)
            )
    ...
    def retrieve_batch_results(self, batch_id: str) -> List[Dict]:
        ...
        if batch_job.status == "completed":
            ...
            for line in result_file.text.strip().split("\n"):
                ...
                analysis = result["response"]["body"]["choices"][0]["message"][
                    "content"
                ]
                ...
                try:
                    ...
                    results.append(
                        {"frame_id": result["custom_id"], "tags": parsed_tags}
                    )
...
class VideoTaggingAutomation:
        ...
        try:
            ...
            for video in recent_videos:
                try:
                    ...
                    self.logger.error(
                        f"Failed to process video {video['Name']}: {str(e)}"
                    )
    ...
    def _process_single_video(self, video: Dict) -> bool:
        ...
        try:
            ...
            batch_id = self.vision_processor.submit_batch_job(
            ...
            self._update_task_status(
                video_id, TaskStatus.COMPLETED, tag_count=len(frame_paths)
            )
    ...
    def _should_process_video(self, video: Dict) -> bool:
        ...
        if (
            not video_path.exists() or video_path.stat().st_size < 10 * 1024 * 1024
    ...
    def _update_task_status(
        self, video_id: str, status: TaskStatus, tag_count: int = 0, error: str = None
    ):
